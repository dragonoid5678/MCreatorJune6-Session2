...
import net.mcreator.ourmod.block.GraasssBlock;
...
public class BlepBiomeBiome extends OurmodModElements.ModElement {
	@ObjectHolder("ourmod:blep_biome")
	public static final CustomBiome biome = null;
	public BlepBiomeBiome(OurmodModElements instance) {
		super(instance, 54);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.4f).scale(0.7f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NONE).waterColor(-16711936).waterFogColor(-16711936)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(GraasssBlock.block.getDefaultState(),
							Blocks.GRANITE.getDefaultState(), Blocks.GRANITE.getDefaultState())));
			setRegistryName("blep_biome");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == GraasssBlock.block.getDefaultState().getBlock() || ground == Blocks.GRANITE.getDefaultState().getBlock())
							&& (ground2 == GraasssBlock.block.getDefaultState().getBlock()
									|| ground2 == Blocks.GRANITE.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), Blocks.GRANITE.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == RainbowTreeBlock.block.getDefaultState().getBlock()
					...
					|| blockType == GraasssBlock.block.getDefaultState().getBlock() || blockType == Blocks.GRANITE.getDefaultState().getBlock();
		}
